from transformers import DistilBertConfig, DistilBertTokenizer, DistilBertForSequenceClassification, BertTokenizerFast, \
         GPT2ForSequenceClassification, GPT2Tokenizer, GPT2Config, BertConfig, BertForSequenceClassification, RobertaForSequenceClassification, RobertaConfig
import torch
import numpy as np
import datasets
...
class Bert():
    def __init__(self, dim: int=768, hidden_dim: int=3072, n_heads: int=12, n_layers: int=12, pretrained=False) -> None:
        ...
        if not pretrained:
            ...
            self.model.bert.encoder.layer = self.model.bert.encoder.layer[:n_layers]
...
class Trainer():
    def __init__(self, ds: datasets.dataset_dict.DatasetDict,
        ...
        if tokenization_required:
            self.ds_tokenized = ds.map(self.preprocess_function, batched=True)
        else:
